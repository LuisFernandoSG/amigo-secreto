...
export const LandingPage = () => {
  ...
  const [activePanel, setActivePanel] = useState("join");
  ...
  const onCreate = handleCreateSubmit(async (values) => {
    ...
    try {
      ...
      rememberParticipantAccess(
        data.hostParticipant.id,
        data.hostParticipant.accessCode
      );
      ...
      setCreateFeedback(
        error.message || "No pudimos crear el grupo. Inténtalo de nuevo."
      );
    } finally {
      setIsCreating(false);
    }
  ...
  const onJoin = handleJoinSubmit(async (values) => {
    ...
    try {
      ...
      setJoinFeedback(
        error.message ||
          "No pudimos unirte al grupo. Revisa el código e inténtalo de nuevo."
      );
    } finally {
      setIsJoining(false);
    }
  ...
  const hasSavedRooms =
    savedHosts.length > 0 || savedParticipantLinks.length > 0;

  const steps = [
    {
      title: "Crea o únete",
      description:
        "Cada invitado recibe un código único y seguro para administrar su lista desde el celular.",
      icon: "🧑‍🎄",
    },
    {
      title: "Comparte deseos",
      description:
        "Las listas son privadas: solo tú las editas y tus amigos solo las leen para inspirarse.",
      icon: "🎁",
    },
    {
      title: "Magia al revelar",
      description:
        "El anfitrión decide cuándo aparece cada amigo secreto y puede cerrar la sala en un clic.",
      icon: "✨",
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <Card
        title="Comencemos"
        description="Un espacio pensado para familias y amigos, con un toque navideño en cada paso."
      >
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          {steps.map((step) => (
            ...
            >
              ...
              <h3 className="text-base font-semibold text-moss-900">
                {step.title}
              </h3>
      ...
      <Card
        ...
        description="Alterna entre crear una sala o unirte a una existente."
      >
          ...
          {activePanel === "create" ? (
            <form className="flex flex-col gap-3" onSubmit={onCreate}>
              <TextField
                ...
                placeholder="Familia Alfaro Nuñez"
                {...registerCreate("name", {
                  required: "Ponle nombre a tu grupo",
                })}
              ...
              <TextField
                ...
                {...registerCreate("ownerName", {
                  required: "Cuéntanos quién organiza",
                })}
              ...
              <Button type="submit" disabled={isCreating}>
                {isCreating ? (
                  <Loader label="Creando tu sala…" />
                ) : (
                  "Crear sala navideña"
                )}
              </Button>
              {createFeedback ? (
                <p className="text-xs text-holly-700">{createFeedback}</p>
              ) : null}
            </form>
          ) : (
            <form className="flex flex-col gap-3" onSubmit={onJoin}>
              <TextField
                ...
                {...registerJoin("joinCode", {
                  required: "Escribe el código recibido",
                })}
              ...
              {joinFeedback ? (
                <p className="text-xs text-holly-700">{joinFeedback}</p>
              ) : null}
            </form>
          )}
        </div>
                  ...
                  {savedHosts.map((group) => (
                    ...
                    >
                      <div className="flex flex-col">
                        <span className="text-xs uppercase tracking-wide text-moss-500">
                          {group.joinCode}
                        </span>
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row">
                        <Button
                          ...
                          variant="secondary"
                          onClick={() =>
                            navigate(`/grupos/${group.joinCode}`, {
                              state: { adminCode: group.adminCode },
                            })
                          }
                        >
                          Panel de anfitrión
                        ...
                        {group.ownerParticipantId && group.ownerAccessCode ? (
                          <Button
                            ...
                            onClick={() =>
                              navigate(
                                `/grupos/${group.joinCode}/participantes/${group.ownerParticipantId}`,
                                {
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-moss-500">
                          Ingresar como {entry.participantName}
                        </span>
                      </div>
                      <Button
                        variant="secondary"
                        onClick={() =>
                          navigate(
                            `/grupos/${entry.joinCode}/participantes/${entry.participantId}`,
                            {
